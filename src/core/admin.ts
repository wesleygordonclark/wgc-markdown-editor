...
const titleEl = $("#fm-title") as HTMLInputElement;
const dateEl = $("#fm-date") as HTMLInputElement;
const descEl = $("#fm-description") as HTMLInputElement;
const saveBtn = $("#save");
const delBtn = $("#delete");
...
const savedTheme = (localStorage.getItem("nova-theme") as "light" | "dark") || "dark";
setTheme(savedTheme);
...
function renderList() {
  ...
  posts
    ...
    .forEach(p => {
      ...
      if (snippet) li.appendChild(snippetDiv);
...
async function loadList() {
  ...
  const metaPosts: Post[] = await res.json();
